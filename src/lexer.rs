...
impl Lexer {
    ...
    fn scan_token(&mut self) -> Result<Token, Diagnostic> {
        ...
        let kind = match ch {
            ...
            c if c.is_ascii_digit() => return Ok(self.number(c, line, column)),
            c if is_ident_start(c) => return Ok(self.identifier(c, line, column)),
            _ => return Err(Diagnostic::new(format!("unexpected character `{ch}`"), line, column)),
        };
    ...
    fn identifier(&mut self, first: char, line: usize, column: usize) -> Token {
        ...
        while self.peek().is_some_and(is_ident_continue) { text.push(self.advance()); }
    ...
    fn number(&mut self, first: char, line: usize, column: usize) -> Token {
        let mut text = String::from(first);
        while self.peek().is_some_and(|c| c.is_ascii_digit()) { text.push(self.advance()); }
        Token { kind: TokenKind::Integer(text.parse().expect("digits form an integer")), line, column }
    }

    fn string(&mut self, line: usize, column: usize) -> Result<Token, Diagnostic> {
        let mut value = String::new();
        while let Some(ch) = self.peek() {
            if ch == '"' { self.advance(); return Ok(Token { kind: TokenKind::String(value), line, column }); }
            if ch == '\n' { return Err(Diagnostic::new("unterminated string", line, column)); }
            if ch == '\\' {
                self.advance();
                let escaped = self.peek().ok_or_else(|| Diagnostic::new("unterminated escape", self.line, self.column))?;
                self.advance();
                value.push(match escaped { 'n' => '\n', 't' => '\t', '"' => '"', '\\' => '\\', other => other });
            } else { value.push(self.advance()); }
        }
        ...
        loop {
            match self.peek() {
                ...
                Some('#') if self.peek_next() == Some('|') => {
                    let line = self.line; let column = self.column;
                    self.advance(); self.advance();
                    while !(self.peek() == Some('|') && self.peek_next() == Some('#')) {
                        if self.at_end() { return Err(Diagnostic::new("unterminated block comment", line, column)); }
                        self.advance();
                    }
                    self.advance(); self.advance();
                }
    ...
    fn advance(&mut self) -> char {
        let ch = self.chars[self.current]; self.current += 1;
    ...
    fn peek(&self) -> Option<char> { self.chars.get(self.current).copied() }
    fn peek_next(&self) -> Option<char> { self.chars.get(self.current + 1).copied() }
    fn at_end(&self) -> bool { self.current >= self.chars.len() }
}

fn is_ident_start(ch: char) -> bool { ch.is_ascii_alphabetic() || ch == '_' }
fn is_ident_continue(ch: char) -> bool { is_ident_start(ch) || ch.is_ascii_digit() }
